...
const RegisterTeacher = () => {
    const {handleRegister,isLogin,handleEmail,handlePass,handleName,error, toggleLogin, professionRef} = useFirebase();
    return (
        <>
        ...
        <h2 style={{fontWeight: 'bold'}}>{isLogin ? 'Login' : 'Register'} as a Teacher: </h2>
            <form className='cam-pavilion-form col' onSubmit={handleRegister}>
                ...
                <p>Profession: <input className='text-input' defaultValue='teacher' readOnly ref={professionRef} type="text"/></p>
