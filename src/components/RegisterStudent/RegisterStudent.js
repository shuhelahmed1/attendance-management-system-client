...
const RegisterStudent = () => {
    const {handleRegister,isLogin,handleEmail,handlePass,handleName,error, toggleLogin, professionRef,attendanceRef, handleAddress} = useFirebase();
    return (
        <>
        ...
        <h2 style={{fontWeight: 'bold'}}>{isLogin ? 'Login' : 'Register'} as a Student: </h2>
            <form className='cam-pavilion-form col' onSubmit={handleRegister}>
                ...
                <p>Attendance: <input className='text-input' defaultValue='present' readOnly ref={attendanceRef} type="text"/></p>
